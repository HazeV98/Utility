...
import { avviaMotoreDashboard } from './dashboard.js';
...
import { initUIDashboard } from './ui_dashboard.js';
...
window.ROTAZIONI_MAP = {
    ...
    "rot_17tr": "Rotazione Linea 17 Tron.", "tc_rot_17tr": "T.C. Rotazione 17 Tronc."
};

const ICON_MAP = {
    dashboard: "fa-solid fa-gauge", calendario: "fa-solid fa-calendar-days", statistiche: "fa-solid fa-chart-simple",
...
const EMOJI_MAP = {
    dashboard: "📟", calendario: "📅", statistiche: "📊", rotazioni: "👥", turni: "🔄",
...
const DEFAULT_APPS = [
    { id: "dashboard", label: "Dashboard", onclick: "window.avviaMotoreDashboardDaIndex()", defaultColor: "#28a745" },
...
window.avviaMotoreOrariDaIndex = () => { avviaMotoreOrari(); };
...
window.LayoutEngine = {
    ...
    render: function() {
        ...
        this.prefs.apps.forEach((app, index) => {
            ...
            if (currentImage) { isImagePath = true; imagePath = currentImage; } 
                ...
                isImagePath = true; imagePath = currentIcon;
            }
            if (isImagePath) { 
    ...
    salvaModificaSingolaApp: function() {
        ...
        if(index > -1) {
            ...
            if (imgData) { this.prefs.apps[index].image = imgData; this.prefs.apps[index].icon = ""; } 
            else { delete this.prefs.apps[index].image; this.prefs.apps[index].icon = document.getElementById('edit-app-icon').value || "🔗"; }
        } 
...
window.apriGestioneAccessi = async () => {
    ...
    try {
        ...
        snap.forEach(d => {
            ...
            u.uid = d.id; window.utentiMap[d.id] = u; window.utentiArrayCache.push(u);
        });
